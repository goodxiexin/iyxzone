...
// edit, update photo
Object.extend(Iyxzone.GuildPhoto, {

  updateMultiplePhotos: function(btn, form, albumID){
    new Ajax.Request(Iyxzone.URL.updateMultipleGuildPhoto(albumID), {
      method: 'put',
      ...
      onSuccess: function(transport){
        ...
        if(json.code == 1){
          window.location.href = Iyxzone.URL.showGuildAlbum(albumID);
        }else{
  ...
  updatePhoto: function(at, btn, form, photoID, albumID){
    ...
    // record new album id
    new Ajax.Request(Iyxzone.URL.updateGuildPhoto(photoID), {
      method: 'put',
      parameters: $(form).serialize(),
      onLoading: function(){
        Iyxzone.disableButton(btn, "请等待..");
      ...
      onSuccess: function(transport){
        ...
        if(json.code == 1){
          if(at == 'album'){
            ...
            window.location.href = Iyxzone.URL.showGuildPhoto(photoID);
          }
        }else if(json.code == 0){
          error("发生错误，请稍后再试");
        }
  ...
  deletePhoto: function(photoID, albumID){
    new Ajax.Request(Iyxzone.URL.deleteGuildPhoto(photoID), {
      method: 'delete',
      onLoading: function(){},
      onComplete: function(){},
      onSuccess: function(transport){
