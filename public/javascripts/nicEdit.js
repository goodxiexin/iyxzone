...
var nicEditorConfig = bkClass.extend({
	...
	buttonList : ['save','bold','italic','underline','left','center','right','justify','ol','ul','fontSize','fontFamily','fontFormat','indent','outdent','emotion','upload','link','unlink','forecolor','bgcolor'],
	iconList : {"bgcolor":1,"forecolor":2,"bold":3,"center":4,"hr":5,"indent":6,"italic":7,"justify":8,"left":9,"ol":10,"outdent":11,"removeformat":12,"right":13,"save":24,"strikethrough":15,"subscript":16,"superscript":17,"ul":18,"underline":19,"image":20,"link":21,"unlink":22,"close":23,"arrow":25,"upload":26, "emotion": 1}
...
var nicEmotionOptions = {
	buttons : {
		'emotion' : {name : 'emotion', type : 'nicEmotionButton'}
	}
	...
	addPane : function(){
		...
		for(var i=0;i<rows;i++){
      			...
      			for(var j=0;j < faces_per_row && i*faces_per_row + j < len;j++){
        			...
        			var face = new Element('img', {src: "/faces/"+(i*faces_per_row + j)+".gif", width: '20px', height: '20px', index: (i*faces_per_row + j)});
        			face.observe('click', function(e){
          				var index = parseInt(e.target.readAttribute('index'));
					var img_url = '/faces/' + index + '.gif';
          				this.ne.nicCommand("insertImage", img_url);
					this.im = this.findElm('IMG','src',img_url);
					this.removePane();
        			}.bind(this));
