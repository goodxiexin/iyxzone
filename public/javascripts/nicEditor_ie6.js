  ...
  paneHTML: " <p class='z-h'>插入图片</p> \
              <div class='z-con'> \
                <div class='content'> \
                  ...
                  <div class='formcontent'>  \
                    <div class='rows s_clear'> \
                      ...
                      <div class='fldvalue'> \
                        <div style='width: 150px;' class='textfield'><input size='30' value='' type='text' id='image_url_field' /></div></div> \
                    </div> \
                  </div> \
                </div> \
                ...
                <div class='z-submit s_clear space'> \
                  <div class='buttons'> \
                    <span class='button'><span><button id='url_image_submit_btn'>插入</button></span></span> \
                    <span class='button button-gray'><span><button  onclick='facebox.close();'>取消</button></span></span> \
                  </div> \
                </div> \
              </div>",
...
var nicEmotionButton = nicEditorAdvancedButton.extend({
  ...
  options: {iconFiles :  '/images/faces/' + encodeURIComponent('亲亲') +'.gif'},
    ...
    for(var i=0;i<len;i++){
      var f = this.symbols[i];
      var a = new bkElement('a');
      a.setAttributes({'href': 'javascript: void(0)', 'title': f}); 
      var img = new bkElement('img');
      img.setAttributes({"src": "/images/faces/" + f.slice(1, f.length - 1) + ".gif", "index": i});
      a.appendChild(img);
      a.addEvent('click', function(e){
        var url = '';
        // target 是 prototype 的方法
        if(navigator.userAgent.indexOf("MSIE") > 0) {
    ...
    div.appendTo(this.pane.pane);
/*		var len = this.symbols.length;
		var faces_per_row = 6;
		var rows = len/faces_per_row;
    var div = new bkElement('DIV').setStyle({width:'270px'});
		for(var i=0;i<rows;i++){
      			for(var j=0;j < faces_per_row && i*faces_per_row + j < len;j++){
              var f = this.symbols[i*faces_per_row + j].substring(1, this.symbols[i*faces_per_row + j].length - 1);
        			var face = new bkElement('img').setAttributes({'src' : "/images/faces/"+ encodeURIComponent(f) +".gif",'index': (i*faces_per_row + j)}).setStyle({width : '20px', height: '20px'});
              face.addEvent('click', function(e){
                var img_url = "";
                if(navigator.userAgent.indexOf("MSIE")>0) {
                  img_url = e.srcElement.getAttribute('src');
                } 
                else{
                  img_url = e.target.getAttribute('src');
                }
                var tmp = 'javascript:nicImTemp();';
          		  this.ne.nicCommand("insertImage", tmp);
					      this.im = this.findElm('IMG','src',tmp);
                if (this.im) {
                  this.im.setAttributes( {src :img_url});
                }
					      this.removePane();
                this.ne.nicCommand("Unselect", this.im);
        		  }.closure(this));
            ...
            var br = new bkElement('br');
            br.appendTo(div);
	    	}
    div.noSelect().appendTo(this.pane.pane);*/
	}
});
