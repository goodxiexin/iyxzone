class FriendGuessesController < ApplicationController
  layout 'app'
  ...
  def show_all_server_characters
    @user = current_user
    @common_game_characters2={}
    current_user.characters.each do |game_character|
      @current_game_characters2 = {}
      @current_game_characters2.default = ' '
      game_character.server.game_characters.each do |other_character|
        @current_game_characters2[other_character.user.id] += (other_character.name + ' ')
      end
      @common_game_characters2[game_character.server.id] = @current_game_characters2
    end
  end
end
