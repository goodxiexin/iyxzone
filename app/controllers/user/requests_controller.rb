class User::RequestsController < UserBaseController
    ...
    if params[:type].blank?
      ...
      @requests.concat current_user.friend_requests.all(:include => [{:user => :profile}])
      ...
      @requests = current_user.friend_requests.paginate :page => params[:page], :per_page => 10, :include => [{:user => :profile}]
    elsif params[:type].to_i == 2
      @requests = current_user.event_requests.paginate :page => params[:page], :per_page => 10, :include => [:event, {:participant => :profile}]
