...
class BlogObserver < ActiveRecord::Observer
  ...
  def after_create blog
    ...
    # update counter
    if blog.draft
      poster.raw_increment "drafts_count"
    else
      poster.raw_increment "blogs_count#{blog.privilege}"
    end
    ...
    # issue feeds
    if !blog.draft and poster.application_setting.emit_blog_feed? and !blog.is_owner_privilege?
      blog.deliver_feeds
    end
  end
  ...
  def after_update blog
    ...
    if blog.recently_recovered?
      ...
      if blog.draft
        poster.raw_decrement "drafts_count"
      else
        poster.raw_decrement "blogs_count#{blog.privilege}"
      end
      blog.destroy_feeds
    end
    ...
    if blog.draft_was
      ...
      if blog.privilege_changed?
        poster.raw_decrement "blogs_count#{blog.privilege_was}"
        poster.raw_increment "blogs_count#{blog.privilege}"
      end
    end
    ...
    if (blog.draft_was and !blog.is_owner_privilege?) or (blog.was_owner_privilege? and !blog.is_owner_privilege?)
      if poster.application_setting.emit_blog_feed?
        blog.deliver_feeds
      end
    end
