class CommentMailer < ActionMailer::Base
  ...
  def blog_comment(comment, user)
    ...
    subject			"Dayday3 - #{comment.poster.login}在博客#{comment.commentable.title}发表评论"
		body				:user => user, :url => "#{SITE_URL}/blogs/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
  end
  ...
  def video_comment comment, user
    ...
    body        :user => user, :url => "#{SITE_URL}/videos/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
	end
  ...
  def photo_comment comment, user
    ...
    body        :user => user, :url => "#{SITE_URL}/#{comment.commentable.class.to_s.underscore}s/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
	end
  ...
  def poll_comment comment, user
    ...
    body        :user => user, :url => "#{SITE_URL}/polls/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
	end

	def event_comment comment, user
    setup_email user
		subject     "Dayday3 - #{comment.poster.login}在活动#{comment.commentable.title}留言了"
    body        :user => user, :url => "#{SITE_URL}/events/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
	end

	def guild_comment comment, user
    setup_email user
		subject     "Dayday3 - #{comment.poster.login}在工会#{comment.commentable.name}留言了"
    body        :user => user, :url => "#{SITE_URL}/guilds/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
	end

	def profile_comment comment, user
		setup_email	user
		subject			"Dayday3 - #{comment.poster.login}在你的留言版上留言了"
		body				:user => user, :url => "#{SITE_URL}/profiles/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
	end

  def game_comment comment, user
    setup_email user
    subject     "Dayday3 - #{comment.poster.login}在游戏#{comment.commentable.name}的留言版上留言了"
    body        :user => user, :url => "#{SITE_URL}/games/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
  end
  ...
  def application_comment comment, user 
    setup_email user
    subject     "Dayday3 - #{comment.poster.login}在应用#{comment.commentable.name}的留言版上回复了"
    body        :user => user, :url => "#{SITE_URL}/applications/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
  end

  def news_comment comment, user
    setup_email user
    subject     "Dayday3 - #{comment.poster.login}在新闻#{comment.commentable.title}的留言版上回复了"
    body        :user => user, :url => "#{SITE_URL}/news/#{comment.commentable_id}?reply_to=#{comment.poster_id}"
  end
