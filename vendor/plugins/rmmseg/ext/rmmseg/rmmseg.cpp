...
extern "C" {
    ...
    static void w_mark(Word *w)
    {
        // start and end are Fixnums, no need to mark
        rb_gc_mark(w->text);
		}
    static void w_free(Word *w) 
    {
        free(w);
    }

    static VALUE w_text(VALUE self)
    {
        Word *w = (Word *)DATA_PTR(self);
        return w->text;
    }

		static VALUE w_cixing(VALUE self)
		{
			Word *w = (Word *)DATA_PTR(self);
			return w->cixing;
		}

		static VALUE w_freq(VALUE self)
		{
			Word *w = (Word *)DATA_PTR(self);
			return w->freq;
		}		
    ...
    static VALUE dic_has_word(VALUE mod, VALUE word)
    {
		...
		static VALUE dic_get_word(VALUE mod, VALUE word)
		{
			const char *str = RSTRING(word)->ptr;
			int nbytes = RSTRING(word)->len;
			rmmseg::Word *w = rmmseg::dict::get(str, nbytes);
			if(w)
        return w_create(w);
			return Qnil;
		}
